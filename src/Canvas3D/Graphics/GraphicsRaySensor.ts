...
export class GraphicsRaySensor {
	...
	constructor(public graphics: Graphics){
		...
		graphicsCanvas.on("mousemove", (e) => {
			...
			// const mousePoint = graphicsCanvas.computeNormalizedMousePosition(e.clientX, e.clientY);
			...
			isHover = intersects.length > 0 && intersects[0].object.uuid === this._target?.uuid;
